...
class featureDataset_FM_Abl(Dataset):
    def __init__(self, original_dataset, bge_embeds, psycholinguistic_feats):
        self.original_dataset = original_dataset
        self.bge_embeds = bge_embeds
    ...
    def __getitem__(self, idx):
        ...
        item['bge_embedding'] = self.bge_embeds[idx]
...
def load_data_FM_Abl(dataset_name, preprocessing, classes, label_cols, text_col, paraphrased=False, split=False):
    if dataset_name == "Kaggle":
        if preprocessing == "Light":
            ...
            bge_embeddings = torch.load(f"./Features/Kaggle/BGE_kaggle_light.pt")
        elif preprocessing == "Heavy":
            data = pd.read_csv(f"./Data/Kaggle/kaggle_heavy.csv")
            bge_embeddings = torch.load(f"./Features/Kaggle/BGE_kaggle_heavy.pt")
        ...
        if paraphrased and preprocessing == "Light":
            ...
            bge_embeddings_paraphrased = torch.load(f"./Features/Kaggle/BGE_kaggle_light_paraphrased.pt")
        ...
        if split and preprocessing == "Light":
            ...
            bge_embeddings_split = torch.load(f"./Features/Kaggle/BGE_kaggle_light_split.pt")
        ...
        if preprocessing == "Light":
            ...
            bge_embeddings = torch.load(f"./Features/Essays/BGE_essays_light.pt")
        elif preprocessing == "Heavy":
            data = pd.read_csv(f"./Data/Essays/essays_heavy.csv")
            bge_embeddings = torch.load(f"./Features/Essays/BGE_essays_heavy.pt")
        ...
        if paraphrased and preprocessing == "Light":
            ...
            bge_embeddings_paraphrased = torch.load(f"./Features/Essays/BGE_essays_light_paraphrased.pt")
            sentences_paraphrased = data_paraphrased[text_col].tolist()
        ...
        if split and preprocessing == "Light":
            ...
            bge_embeddings_split = torch.load(f"./Features/Essays/BGE_essays_light_split.pt")
            sentences_split = data_split[text_col].tolist()
    ...
    if paraphrased and split and preprocessing == "Light":
        return data, sentences, labels, bge_embeddings, psycholinguistic_feats, data_paraphrased, sentences_paraphrased, bge_embeddings_paraphrased, psycholinguistic_feats_paraphrased, data_split, sentences_split, bge_embeddings_split, psycholinguistic_feats_split
    else:
        return data, sentences, labels, bge_embeddings, psycholinguistic_feats 
