...
#change paths to match your directory structure
...
MAX_SEQ_LEN = 64
MAX_CHUNKS = 35
BERT_NAME = "/kaggle/input/models/andrmorgado/distil/pytorch/distil/1/distilbert-base-uncased" #change to the path of the DistilBert model you are using
batch_size = 1
...
data, sentences, labels, gte_embeddings, psycholinguistic_feats, data_paraphrased, sentences_paraphrased, gte_embeddings_paraphrased, psycholinguistic_feats_paraphrased, data_split, sentences_split, gte_embeddings_split, psycholinguistic_feats_split = load_data_FM_Abl(dataset_name, preprocessing, classes, label_cols, text_col, paraphrased=True, split=True)
...
for (train_idx, test_idx, val_idx) in [(train_indices, test_indices, val_indices)]:
    ...
    test_gte_paraphrased = gte_embeddings_paraphrased[test_indices]
    ...
    test_dataset = featureDataset_FM_Abl(MBTIDataset_FM_Abl(test_sentences, y_test, tokenizer, MAX_SEQ_LEN), test_gte, test_psycholinguistic_feats_fold)
    test_dataset_paraphrased = featureDataset_FM_Abl(MBTIDataset_FM_Abl(test_sentences_paraphrased, y_test, tokenizer, MAX_SEQ_LEN), test_gte_paraphrased, test_psycholinguistic_feats_fold_paraphrased)
    ...
    with torch.no_grad():
        for user_idx, batch in enumerate(test_loader):
            ...
            cls_out, user_embedding = model(input_ids, attention_mask, gte_emb, psych_batch)
